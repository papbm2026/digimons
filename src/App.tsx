              ...
              <Route path="/" element={
                user ? (
                  user.role === 'Admin' || user.role === 'Viewer' ? (
                    ...
                    <CleaningChecklist user={user} logs={cleaningLogs} onAdd={(l) => setCleaningLogs([l, ...cleaningLogs])} onUpdate={(logs) => setCleaningLogs(logs)} />
                  ) : user.role === 'Security' ? (
                    <SecurityPage user={user} logs={secLogs} onAdd={(l) => setSecLogs([l, ...secLogs])} onUpdate={(logs) => setSecLogs(logs)} />
                  ) : (
                    <Dashboard keluhans={keluhans} cleaning={cleaningLogs} maintenance={maintLogs} security={secLogs} />
                  )
                ) : (
                  <PublicKeluhan existingKeluhans={keluhans} onAdd={(k) => setKeluhans([k, ...keluhans])} />
                )
              } />

              <Route path="/public" element={<PublicKeluhan existingKeluhans={keluhans} onAdd={(k) => setKeluhans([k, ...keluhans])} />} />
              <Route path="/login" element={user ? <Navigate to="/" replace /> : <Login onLogin={handleLogin} />} />
              <Route path="/cleaning" element={renderProtectedRoute(CleaningChecklist, { logs: cleaningLogs, onAdd: (l: CleaningLog) => setCleaningLogs([l, ...cleaningLogs]), onUpdate: setCleaningLogs })} />
              <Route path="/maintenance" element={renderProtectedRoute(MaintenancePage, { logs: maintLogs, onAdd: (l: MaintenanceLog) => setMaintLogs([l, ...maintLogs]), onUpdate: setMaintLogs })} />
              <Route path="/security" element={renderProtectedRoute(SecurityPage, { logs: secLogs, onAdd: (l: SecurityLog) => setSecLogs([l, ...secLogs]), onUpdate: setSecLogs })} />
              <Route path="/complaints" element={
                user?.role === 'Admin' ? (
                  <ComplaintsAdmin keluhans={keluhans} onUpdate={(k) => setKeluhans(keluhans.map(item => item.id === k.id ? k : item))} onDelete={(id) => setKeluhans(keluhans.filter(k => k.id !== id))} />
                ) : (
                  <Navigate to="/" replace />
                )
              } />
