...
const ComplaintsAdmin: React.FC<Props> = ({ keluhans, onUpdate, onDelete }) => {
  ...
  const handleDelete = () => {
    if (!selectedK) return;
    if (window.confirm('Hapus keluhan/saran ini?')) {
      onDelete(selectedK.id);
      setSelectedK(null);
    }
  };
        ...
        {selectedK ? (
          <div className="space-y-8">
            <div className="flex items-center justify-between">
              ...
              <button onClick={handleDelete} className="p-3 bg-red-50 text-red-600 rounded-xl hover:bg-red-600 hover:text-white transition-all">
                <Trash2 size={20} />
              </button>
            </div>
