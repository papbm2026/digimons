...
interface Props {
  ...
  onUpdate: (logs: CleaningLog[]) => void;
}
...
const CleaningChecklist: React.FC<Props> = ({ user, logs, onAdd, onUpdate }) => {
  ...
  const validatedMonthlyLogs = useMemo(() => {
    return logs.filter(l => l.tanggal.startsWith(reportMonth) && l.isValidated);
  }, [logs, reportMonth]);
  ...
  const handleDelete = (id: string) => {
    if (window.confirm('Hapus laporan ini? Petugas harus mengisi ulang laporan jika dihapus.')) {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-[2rem] border border-slate-100 shadow-sm overflow-hidden no-print">
        <div className="p-6 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
          <h3 className="font-bold text-slate-800 flex items-center"><Calendar size={18} className="mr-2 text-emerald-600" /> Log Monitoring Harian</h3>
        </div>
                    ...
                    <td className="px-6 py-4">
                      {log ? (
                        <div className="flex flex-col space-y-1">
                          ...
                          <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[9px] font-bold w-fit ${log.isValidated ? 'bg-blue-50 text-blue-700' : 'bg-slate-100 text-slate-400'}`}>
                            {log.isValidated ? 'VALIDATED' : 'WAITING VALIDATION'}
                          </span>
                        </div>
                      ) : (
                    ...
                    <td className="px-6 py-4">
                      {log && isAdmin && (
                        <div className="flex items-center space-x-2">
                          {!log.isValidated && (
                            <button onClick={() => handleValidate(log.id)} className="p-2 bg-emerald-100 text-emerald-600 rounded-lg hover:bg-emerald-600 hover:text-white transition-colors" title="Validasi">
                              <ShieldCheck size={16} />
                            </button>
                          )}
