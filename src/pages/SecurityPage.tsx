...
import { 
  ...
  PackageSearch, Car, Eye, ShieldAlert
} from 'lucide-react';
...
interface Props {
  ...
  onUpdate: (logs: SecurityLog[]) => void;
}

const SecurityPage: React.FC<Props> = ({ user, logs, onAdd, onUpdate }) => {
  ...
  const usedAreas = useMemo(() => {
    return logs.filter(l => l.tanggal === today && l.shift === formData.shift).map(l => l.area);
  }, [logs, today, formData.shift]);

  const validatedMonthlyLogs = useMemo(() => {
    return logs.filter(l => l.tanggal.startsWith(reportMonth) && l.isValidated).sort((a, b) => b.tanggal.localeCompare(a.tanggal));
  }, [logs, reportMonth]);
  ...
  const handleValidate = (id: string) => {
    onUpdate(logs.map(l => l.id === id ? { ...l, isValidated: true } : l));
  };

  const handleDelete = (id: string) => {
    if (window.confirm('Hapus laporan keamanan ini?')) {
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center space-x-3">
          {isAdmin && (
            <div className="flex items-center bg-white border border-slate-200 rounded-xl px-2">
              ...
              <button onClick={() => window.print()} className="flex items-center space-x-2 px-3 py-2 text-blue-600 hover:bg-blue-50 border-l"><Printer size={18} /><span className="text-xs font-bold">Cetak Rekap Per Area</span></button>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-10">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-3 gap-3">
                  {(['Pagi', 'Siang', 'Malam'] as const).map(s => (
                    <button key={s} type="button" onClick={() => setFormData({ ...formData, shift: s, area: '' })} className={`flex flex-col items-center p-4 rounded-2xl border-2 transition-all ${formData.shift === s ? 'bg-blue-600 text-white' : 'bg-white border-slate-100 text-slate-400'}`}>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {availableAreas.map(area => {
                  ...
                  return (
                    <button key={area} type="button" disabled={isTaken} onClick={() => setFormData({ ...formData, area: area })} className={`relative flex flex-col items-center p-5 rounded-3xl border-2 transition-all ${formData.area === area ? 'bg-blue-50 border-blue-600 text-blue-900' : isTaken ? 'bg-slate-50 text-slate-300' : 'bg-white border-slate-100 text-slate-500 hover:border-blue-300'}`}>
            ...
            {formData.area === 'Tunggu Sidang' && (
              <div className="space-y-4 p-6 bg-slate-50 rounded-[2rem] border border-slate-200 animate-in fade-in duration-300">
                <label className="block text-xs font-bold text-blue-600 uppercase flex items-center">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <button type="button" onClick={() => setFormData({...formData, isSterilisasi: !formData.isSterilisasi})} className={`flex items-center space-x-3 p-4 rounded-2xl border-2 transition-all ${formData.isSterilisasi ? 'bg-white border-blue-500 text-blue-900 shadow-sm' : 'bg-slate-100/50 border-slate-200 text-slate-400'}`}>
                    ...
                    <div className="text-left"><p className="text-xs font-bold">Sterilisasi Ruang Sidang</p><p className="text-[10px] opacity-70">Tidak ada barang berbahaya</p></div>
                  </button>
                  <button type="button" onClick={() => setFormData({...formData, isPengaturanPengunjung: !formData.isPengaturanPengunjung})} className={`flex items-center space-x-3 p-4 rounded-2xl border-2 transition-all ${formData.isPengaturanPengunjung ? 'bg-white border-blue-500 text-blue-900 shadow-sm' : 'bg-slate-100/50 border-slate-200 text-slate-400'}`}>
                    ...
                    <div className="text-left"><p className="text-xs font-bold">Pengaturan Pengunjung Sidang</p><p className="text-[10px] opacity-70">Tertib (No Kegaduhan & Smoking)</p></div>
                  </button>
                  <button type="button" onClick={() => setFormData({...formData, isAksesPihak: !formData.isAksesPihak})} className={`flex items-center space-x-3 p-4 rounded-2xl border-2 transition-all ${formData.isAksesPihak ? 'bg-white border-blue-500 text-blue-900 shadow-sm' : 'bg-slate-100/50 border-slate-200 text-slate-400'}`}>
                    ...
                    <div className="text-left"><p className="text-xs font-bold">Pemeriksaan Pihak/Pengunjung</p><p className="text-[10px] opacity-70">Identitas, Kartu & Barang Bawaan</p></div>
                  </button>
                  <button type="button" onClick={() => setFormData({...formData, isAreaTerbatas: !formData.isAreaTerbatas})} className={`flex items-center space-x-3 p-4 rounded-2xl border-2 transition-all ${formData.isAreaTerbatas ? 'bg-white border-blue-500 text-blue-900 shadow-sm' : 'bg-slate-100/50 border-slate-200 text-slate-400'}`}>
                    ...
                    <div className="text-left"><p className="text-xs font-bold">Pengamanan Akses Area Terbatas</p><p className="text-[10px] opacity-70">Sterilisasi Jalur Pegawai</p></div>
                  </button>
                </div>
              </div>
            )}
            ...
            {formData.area === 'Pos Depan' && (
              <div className="space-y-4 p-6 bg-slate-50 rounded-[2rem] border border-slate-200 animate-in fade-in duration-300">
                <label className="block text-xs font-bold text-emerald-600 uppercase flex items-center">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <button type="button" onClick={() => setFormData({...formData, isIdentifikasiPengunjung: !formData.isIdentifikasiPengunjung})} className={`flex flex-col items-center space-y-3 p-5 rounded-2xl border-2 transition-all ${formData.isIdentifikasiPengunjung ? 'bg-white border-emerald-500 text-emerald-900 shadow-sm' : 'bg-slate-100/50 border-slate-200 text-slate-400'}`}>
                    ...
                    <div className="text-center"><p className="text-xs font-bold">Identifikasi Pengunjung</p><p className="text-[9px] opacity-70">Buku Tamu & Kartu Pengunjung</p></div>
                  </button>
                  <button type="button" onClick={() => setFormData({...formData, isPeriksaBarang: !formData.isPeriksaBarang})} className={`flex flex-col items-center space-y-3 p-5 rounded-2xl border-2 transition-all ${formData.isPeriksaBarang ? 'bg-white border-emerald-500 text-emerald-900 shadow-sm' : 'bg-slate-100/50 border-slate-200 text-slate-400'}`}>
                    ...
                    <div className="text-center"><p className="text-xs font-bold">Pemeriksaan Barang</p><p className="text-[9px] opacity-70">Cek Barang Bawaan Tamu</p></div>
                  </button>
                  <button type="button" onClick={() => setFormData({...formData, isPeriksaKendaraan: !formData.isPeriksaKendaraan})} className={`flex flex-col items-center space-y-3 p-5 rounded-2xl border-2 transition-all ${formData.isPeriksaKendaraan ? 'bg-white border-emerald-500 text-emerald-900 shadow-sm' : 'bg-slate-100/50 border-slate-200 text-slate-400'}`}>
                    ...
                    <div className="text-center"><p className="text-xs font-bold">Pemeriksaan Kendaraan</p><p className="text-[9px] opacity-70">Pengecekan Kendaraan Tamu</p></div>
                  </button>
                </div>
              </div>
            )}

            {/* Checklist Khusus PTSP */}
            {formData.area === 'PTSP' && (
              <div className="space-y-4 p-6 bg-slate-50 rounded-[2rem] border border-slate-200 animate-in fade-in duration-300">
                <label className="block text-xs font-bold text-amber-600 uppercase flex items-center">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <button type="button" onClick={() => setFormData({...formData, isPeriksaPengunjungPTSP: !formData.isPeriksaPengunjungPTSP})} className={`flex flex-col items-center space-y-3 p-5 rounded-2xl border-2 transition-all ${formData.isPeriksaPengunjungPTSP ? 'bg-white border-amber-500 text-amber-900 shadow-sm' : 'bg-slate-100/50 border-slate-200 text-slate-400'}`}>
                    ...
                    <div className="text-center"><p className="text-xs font-bold">Pemeriksaan Pengunjung</p><p className="text-[9px] opacity-70">Barang & Kartu Tamu</p></div>
                  </button>
                  <button type="button" onClick={() => setFormData({...formData, isAksesKeluarMasukPTSP: !formData.isAksesKeluarMasukPTSP})} className={`flex flex-col items-center space-y-3 p-5 rounded-2xl border-2 transition-all ${formData.isAksesKeluarMasukPTSP ? 'bg-white border-amber-500 text-amber-900 shadow-sm' : 'bg-slate-100/50 border-slate-200 text-slate-400'}`}>
                    ...
                    <div className="text-center"><p className="text-xs font-bold">Pengawasan Akses</p><p className="text-[9px] opacity-70">Monitor Keluar-Masuk Pihak</p></div>
                  </button>
                  <button type="button" onClick={() => setFormData({...formData, isPengaturanPengunjungPTSP: !formData.isPengaturanPengunjungPTSP})} className={`flex flex-col items-center space-y-3 p-5 rounded-2xl border-2 transition-all ${formData.isPengaturanPengunjungPTSP ? 'bg-white border-amber-500 text-amber-900 shadow-sm' : 'bg-slate-100/50 border-slate-200 text-slate-400'}`}>
                    ...
                    <div className="text-center"><p className="text-xs font-bold">Pengaturan Pengunjung</p><p className="text-[9px] opacity-70">Tertib & No Smoking</p></div>
                  </button>
                </div>
              </div>
            )}
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <button type="button" onClick={() => setFormData({ ...formData, statusAman: true, keteranganTidakAman: '' })} className={`flex items-center justify-center space-x-3 p-4 rounded-2xl border-2 transition-all ${formData.statusAman ? 'bg-emerald-50 border-emerald-500 text-emerald-900 shadow-sm' : 'bg-white border-slate-100'}`}>
                  <CheckCircle2 size={24} className={formData.statusAman ? 'text-emerald-600' : 'text-slate-300'} /><span className="font-bold">Aman</span>
                </button>
                <button type="button" onClick={() => setFormData({ ...formData, statusAman: false })} className={`flex items-center justify-center space-x-3 p-4 rounded-2xl border-2 transition-all ${!formData.statusAman ? 'bg-red-50 border-red-500 text-red-900 shadow-sm' : 'bg-white border-slate-100'}`}>
                  <AlertCircle size={24} className={!formData.statusAman ? 'text-red-600' : 'text-slate-300'} /><span className="font-bold">Tidak Aman</span>
                </button>
              </div>
            </div>
            <button type="submit" disabled={!formData.area} className={`w-full py-5 rounded-2xl font-bold shadow-xl transition-all ${!formData.area ? 'bg-slate-100 text-slate-400' : 'bg-blue-800 text-white shadow-blue-200'}`}>Simpan Laporan Keamanan</button>
          </form>
        </div>
      )}
          ...
          <table className="w-full text-left">
            <thead className="bg-slate-50 text-[10px] text-slate-400 uppercase tracking-widest">
              <tr>
                ...
                <th className="px-8 py-5">Area & Status</th>
                <th className="px-8 py-5">Validasi</th>
            ...
            <tbody className="divide-y divide-slate-100 text-sm">
              {logs.map(l => (
                <tr key={l.id} className="hover:bg-slate-50/50">
                  <td className="px-8 py-6"><p className="font-bold text-slate-700">{l.tanggal}</p><span className="px-2 py-0.5 rounded-full bg-blue-50 text-blue-700 text-[9px] font-bold uppercase">{l.shift}</span></td>
                  <td className="px-8 py-6 font-bold">{l.petugas}</td>
                  <td className="px-8 py-6">
                    <p className="font-bold text-slate-800">{l.area}</p>
                    <span className={`text-[9px] font-bold ${l.statusAman ? 'text-emerald-600' : 'text-red-600'}`}>{l.statusAman ? 'KONDISI AMAN' : 'KONDISI TIDAK AMAN'}</span>
                  </td>
                  <td className="px-8 py-6">
                    <span className={`inline-block px-3 py-1 rounded-full text-[10px] font-bold ${l.isValidated ? 'bg-blue-50 text-blue-700' : 'bg-slate-100 text-slate-400'}`}>{l.isValidated ? 'VALIDATED' : 'WAITING VALIDATION'}</span>
                  </td>
                  <td className="px-8 py-6">
                    {isAdmin && (
                      <div className="flex items-center space-x-2">
                        {!l.isValidated && (
                          /* Fixed 'id' to 'l.id' */
                          <button onClick={() => handleValidate(l.id)} className="p-2 bg-blue-100 text-blue-600 rounded-lg hover:bg-blue-600 hover:text-white transition-colors"><ShieldCheck size={16} /></button>
                        )}
                        <button onClick={() => handleDelete(l.id)} className="p-2 bg-red-100 text-red-600 rounded-lg hover:bg-red-600 hover:text-white transition-colors"><Trash2 size={16} /></button>
                      </div>
                    )}
                  </td>
                </tr>
      ...
      <div className="print-only p-8">
        <div className="text-center mb-10 border-b-4 border-black pb-8">
          ...
          <p className="text-md mt-2">Periode: {new Date(reportMonth + "-01").toLocaleString('id-ID', { month: 'long', year: 'numeric' })} (Hanya Data Valid)</p>
        </div>
          ...
          return (
            <div key={areaName} className="mb-12 break-inside-avoid">
              <div className="bg-slate-100 p-2 border-2 border-black mb-1">
                <h3 className="text-sm font-bold uppercase">Area Kerja: {areaName}</h3>
              </div>
              <table className="w-full border-collapse border-2 border-black text-[9px]">
                <thead>
                  <tr className="bg-slate-50">
                    ...
                    <th className="border border-black p-2 w-16">Shift</th>
                    <th className="border border-black p-2 w-24">Petugas</th>
                    <th className="border border-black p-2">Detail Keamanan / Checklist</th>
                  ...
                  {areaLogs.length > 0 ? (
                    areaLogs.map(l => (
                      <tr key={l.id}>
                        ...
                        <td className="border border-black p-2 text-center font-bold">{l.shift}</td>
                        <td className="border border-black p-2 text-center">{l.petugas}</td>
                        <td className="border border-black p-2">
                          <div className="space-y-1">
                            {l.area === 'Tunggu Sidang' ? (
                              <>
                                <div className="flex items-center justify-between"><span>1. Sterilisasi Ruang Sidang (Barang Berbahaya)</span><span className="font-bold">{l.isSterilisasi ? '[V]' : '[ ]'}</span></div>
                                <div className="flex items-center justify-between"><span>2. Pengaturan Pengunjung Sidang (No Kegaduhan)</span><span className="font-bold">{l.isPengaturanPengunjung ? '[V]' : '[ ]'}</span></div>
                                <div className="flex items-center justify-between"><span>3. Pemeriksaan Pihak (Identitas, Kartu, Barang)</span><span className="font-bold">{l.isAksesPihak ? '[V]' : '[ ]'}</span></div>
                                <div className="flex items-center justify-between"><span>4. Pengamanan Akses ke Area Terbatas</span><span className="font-bold">{l.isAreaTerbatas ? '[V]' : '[ ]'}</span></div>
                              </>
                            ) : l.area === 'Pos Depan' ? (
                              <>
                              ...
                              <>
                                <div className="flex items-center justify-between"><span>1. Pemeriksaan Pengunjung (Barang/Kartu)</span><span className="font-bold">{l.isPeriksaPengunjungPTSP ? '[V]' : '[ ]'}</span></div>
                                <div className="flex items-center justify-between"><span>2. Pengawasan Akses Pihak</span><span className="font-bold">{l.isAksesKeluarMasukPTSP ? '[V]' : '[ ]'}</span></div>
                                <div className="flex items-center justify-between"><span>3. Pengaturan Pengunjung (Tertib)</span><span className="font-bold">{l.isPengaturanPengunjungPTSP ? '[V]' : '[ ]'}</span></div>
                              </>
                            ) : (
                              <>
                                <div className="flex items-center justify-between"><span>1. Lampu Padam Sesuai Jadwal</span><span className="font-bold">{l.isLampuMati ? '[V]' : '[ ]'}</span></div>
                                <div className="flex items-center justify-between"><span>2. Perangkat Elektronik Non-Server Off</span><span className="font-bold">{l.isElektronikOff ? '[V]' : '[ ]'}</span></div>
                                <div className="flex items-center justify-between"><span>3. Pagar & Pintu Terkunci Gembok</span><span className="font-bold">{l.isPagarGembok ? '[V]' : '[ ]'}</span></div>
                                <div className="flex items-center justify-between"><span>4. Lampu Luar/Sorot Menyala</span><span className="font-bold">{l.isLampuLuarHidup ? '[V]' : '[ ]'}</span></div>
                              </>
                            )}
                          </div>
                        </td>
                    ...
                    <tr>
                      <td colSpan={5} className="border border-black p-4 text-center text-slate-400 italic">Tidak ada data laporan tervalidasi untuk area ini pada bulan tersebut.</td>
                    </tr>
                  )}
                </tbody>
              </table>
        ...
        <div className="mt-16 text-sm">
          <div className="grid grid-cols-2 text-center">
            ...
            <div>
              ...
              <p className="font-bold">Sekretaris Pengadilan Agama Prabumulih,</p>
              <div className="h-24"></div>
              <p className="font-bold underline">([Nama-Pegawai])</p>
              <p>NIP. ([NIP_Pegawai])</p>
            </div>
