...
const BANNED_WORDS = [
  'bodoh', 'tolol', 'goblok', 'anjing', 'babi', 'bangsat', 'asu', 'kontol', 'memek', 
  'jembut', 'ngentot', 'itil', 'perek', 'lonte', 'bajingan', 'peju', 'bencong', 
  'banci', 'setan', 'iblis', 'kampret', 'brengsek', 'tai', 'bego'
];
...
const PublicKeluhan: React.FC<PublicKeluhanProps> = ({ onAdd, existingKeluhans }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (validateContent(formData.deskripsi) || validateContent(formData.pelapor) || validateContent(formData.lokasi)) {
      setError("Maaf, laporan Anda mengandung kata-kata yang tidak pantas.");
      return;
    }

    if (isDuplicate()) {
      setError("Laporan yang sama sudah dikirimkan hari ini.");
      return;
    }
    ...
    // Logic for dynamic PIC based on category and sub-category
    let waNumber = '';
    let picName = '';
    ...
    if (category === 'Kebersihan') {
      ...
      if (formData.subKategori === 'Gedung') {
        ...
        // Peralatan Non TIK or Peralatan TIK
    ...
    <div className="max-w-4xl mx-auto py-12 px-4 min-h-[80vh] flex flex-col justify-center">
      ...
      {step === 'form' && (
        <div className="bg-white p-8 md:p-12 rounded-[2rem] shadow-2xl border border-slate-100 animate-in fade-in zoom-in-95 duration-500 max-w-2xl mx-auto w-full">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            >
              <Send size={20} />
              <span>Simpan Laporan Digimons</span>
            </button>
          </form>
        </div>
      )}
      ...
      {step === 'success' && (
        <div className="bg-white p-12 rounded-[2.5rem] shadow-2xl border border-slate-100 animate-in zoom-in-95 duration-500 text-center max-w-xl mx-auto w-full">
          ...
          <p className="text-slate-500 mb-10">Laporan Anda telah masuk ke sistem monitoring internal.</p>
          ...
          <div className="flex flex-col space-y-4">
            ...
            >
              ...
              <span>Kirim Notifikasi ke PIC WhatsApp</span>
            </a>
            <button
              onClick={resetForm}
