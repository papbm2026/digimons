...
import { AlertCircle, CheckCircle2, Clock, Shield, Activity, Calendar, MessageSquare, Wrench, Briefcase, User, Car, Bike, AlertTriangle } from 'lucide-react';
...
const Dashboard: React.FC<DashboardProps> = ({ keluhans, cleaning, maintenance, security }) => {
  ...
  const todayCleaning = cleaning.filter(c => c.tanggal === todayStr).length;
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="bg-white px-4 py-2 rounded-xl shadow-sm border border-slate-100 flex items-center space-x-2 text-emerald-700 font-medium">
          <Calendar size={18} />
          <span>{today.toLocaleDateString('id-ID', { dateStyle: 'long' })}</span>
        </div>
      </div>
