...
import { Wrench, Plus, Printer, History, Camera, AlertCircle, X, CheckCircle2, ShieldCheck, Trash2, Coins, MapPin, Tag } from 'lucide-react';
...
const MaintenancePage: React.FC<Props> = ({ user, logs, onAdd, onUpdate }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setFormData({ item: MAINTENANCE_ITEMS[0] as MaintenanceLog['item'], merekArea: '', deskripsiKerusakan: '', detailPerbaikan: '', foto: '' });
    setShowForm(false);
  };
  ...
  const totalBiayaValidated = useMemo(() => {
    return validatedMonthlyLogs.reduce((acc, curr) => acc + (curr.biaya || 0), 0);
  }, [validatedMonthlyLogs]);

  const handleValidate = (id: string) => {
    onUpdate(logs.map(l => l.id === id ? { ...l, isValidated: true } : l));
  };

  const handleUpdateBiaya = (id: string, value: string) => {
    const numericValue = parseInt(value.replace(/[^0-9]/g, '')) || 0;
    onUpdate(logs.map(l => l.id === id ? { ...l, biaya: numericValue } : l));
  };

  const handleDelete = (id: string) => {
    if (window.confirm('Hapus laporan pemeliharaan ini?')) {
          ...
          {isAdmin && (
            <div className="flex items-center bg-white border border-slate-200 rounded-xl px-2">
              ...
              <button onClick={() => window.print()} className="flex items-center space-x-2 px-3 py-2 text-amber-600 hover:bg-amber-50 border-l">
                <Printer size={18} />
                <span className="text-xs font-bold">Cetak Rekap Valid</span>
              </button>
            </div>
          )}
          ...
          <table className="w-full text-left">
            <thead className="bg-slate-50 text-[10px] text-slate-400 uppercase tracking-widest">
              <tr>
                ...
                <th className="px-8 py-5">Biaya (Rp)</th>
            ...
            <tbody className="divide-y divide-slate-100 text-sm">
              {logs.map(l => (
                <tr key={l.id} className="hover:bg-slate-50/50 group transition-colors">
                  ...
                  <td className="px-8 py-6">
                    {isAdmin && !l.isValidated ? (
                      <div className="relative max-w-[150px]">
                        <div className="absolute inset-y-0 left-0 pl-2.5 flex items-center pointer-events-none text-amber-500 font-bold text-[10px]">Rp</div>
                        <input 
                          type="text"
                          inputMode="numeric"
                          className="w-full pl-8 pr-2 py-1.5 rounded-lg bg-white border border-slate-200 text-xs font-bold text-slate-700 focus:border-amber-500 outline-none"
                          value={l.biaya || ''}
                          onChange={(e) => handleUpdateBiaya(l.id, e.target.value)}
                          placeholder="0"
                        />
                      </div>
                    ) : (
                      <span className={`font-bold ${l.biaya && l.biaya > 0 ? 'text-emerald-700' : 'text-slate-300 italic text-xs'}`}>
                        {l.biaya && l.biaya > 0 ? formatIDR(l.biaya) : 'Belum diisi'}
                      </span>
                    )}
                  </td>
                  <td className="px-8 py-6">
                    <span className={`inline-flex items-center px-3 py-1 rounded-full text-[10px] font-bold ${l.isValidated ? 'bg-blue-50 text-blue-700' : 'bg-slate-100 text-slate-400'}`}>
                      {l.isValidated ? <ShieldCheck size={12} className="mr-1.5" /> : null}
                      {l.isValidated ? 'VALIDATED' : 'WAITING VALIDATION'}
                    </span>
                  </td>
                  <td className="px-8 py-6">
                    {isAdmin && (
              ...
              {logs.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-8 py-12 text-center text-slate-400 italic font-medium">Belum ada laporan pemeliharaan</td>
                </tr>
              )}
            </tbody>
          </table>
        ...
        <table className="w-full border-collapse border-2 border-black text-[10px]">
          <thead>
            <tr className="bg-slate-100">
              ...
              <th className="border border-black p-3 w-24">Biaya (Rp)</th>
          ...
          <tbody>
            {validatedMonthlyLogs.map(l => (
              <tr key={l.id}>
                ...
                <td className="border border-black p-3 text-right font-bold">{formatIDR(l.biaya || 0)}</td>
          ...
          <tfoot>
            <tr className="bg-slate-50">
              <td colSpan={5} className="border border-black p-3 text-right font-bold uppercase">Total Biaya Pemeliharaan</td>
              <td className="border border-black p-3 text-right font-bold text-lg">{formatIDR(totalBiayaValidated)}</td>
